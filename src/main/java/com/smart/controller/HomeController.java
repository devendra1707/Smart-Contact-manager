...
public class HomeController {
	...
	public String SignUp(Model model) {
		...
		model.addAttribute("user", new User());   //we send user blank field
		return "signup";
	}
	...
	public String registerUser(@Valid @ModelAttribute("user") User user, BindingResult result1,
			@RequestParam(value = "terms", defaultValue = "false") boolean terms, Model model
			,HttpSession session) {
	if(result1.hasErrors()) {
		...
		try {
			if (!terms) {   //if user cant check term and condtion
			...
			session.setAttribute("message",
					new Message("Something went Wrong !!" + e.getMessage(), "alert-warning"));
			return "signup";
		}
